...
class UnigramFeatureExtractor(FeatureExtractor):
    ...
    def __init__(self, indexer: Indexer):
        ...
        self.stopwords = set(stopwords.words('english')) # stop words from nltk
    ...
    def extract_features(self, sentence: List[str], add_to_indexer: bool=False) -> Counter:
        ...
        for word in sentence:
            ...
            # ignore unseen test words
            if idx != -1:   
                feats[idx] += 1 # increases counter
...
class PerceptronClassifier(SentimentClassifier):
    ...
    def predict(self, sentence: List[str]) -> int:
        ...
        score = sum(self.weights[idx] * val for idx, val in feats.items() if idx < len(self.weights))
...
def train_perceptron(train_exs: List[SentimentExample], feat_extractor: FeatureExtractor) -> PerceptronClassifier:
    ...
    for i in range(epochs):
        ...
        for j in train_exs:
            # feature vector
            feats = feat_extractor.extract_features(j.words)
            ...
            if pred != j.label:
                gold = 1 if j.label == 1 else -1
                for f, v in feats.items():
                    w[f] = weigwhts.get(f, 0.0) + gold * v

    return PerceptronClassifier(w, feat_extractor)
